...
# Add Master IP and HOSTNAME to /srv/ftp/hostsname
echo  "`ifconfig | grep Bcast | cut -d : -f 2 | cut -d ' ' -f 1` `hostname`" | sudo  tee  /srv/ftp/hostsname


# Add Slave IP and HOSTNAME to /srv/ftp/hostsname
...
until [ "$STOP" = "stop" ]; 
do
        echo "== Let's add Slave IP  to /srv/ftp/hostsname =="
        echo "== Let's add Slave HOSTNAME to /srv/ftp/hostsname =="
	echo ""
        echo "== Now, /srv/ftp/hostsname is =="
	cat /srv/ftp/hostsname
	echo ""
	echo "\033[33m"
        ...
        echo "Add $SLAVEIP and $SLAVEHOSTNAME to /srv/ftp/hostsname"
        echo "$SLAVEIP  $SLAVEHOSTNAME"  | sudo tee -a  /srv/ftp/hostsname
	echo ""
...
# Copy /srv/ftp/hostsname  to /etc/hosts
echo "\033[32m"
echo " Copy /srv/ftp/hostsname to /etc/hosts"
echo "\033[0m"
sudo cp  /srv/ftp/hostsname  /etc/hosts
...
ssh-keyscan   `ifconfig | grep Bcast | cut -d : -f 2 | cut -d ' ' -f 1`   >>   ~/.ssh/known_hosts
ssh-keyscan   0.0.0.0   >>   ~/.ssh/known_hosts
ssh-keyscan   127.255.255.1   >>   ~/.ssh/known_hosts
